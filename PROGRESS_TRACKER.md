...
- [ ] Create `Roles` and `Permissions` schema and initial logic.

## Phase 4: Authentication & Authorization
- [ ] Install `@nestjs/jwt` and `@nestjs/passport` along with `passport-jwt`.
- [ ] Create `Auth` module, service, and controller for login/registration.
- [ ] Implement `JwtAuthGuard`.
- [ ] Implement `TenantGuard`.
- [ ] Implement `RolesGuard`.

## Phase 5: CRM Business Modules
- [ ] Create `Leads` module (Schema, Service, Controller).
- [ ] Create `Contacts` module.
- [ ] Create `Customers` module.
- [ ] Create `Deals` module.
- [ ] Create `Quotes` module.
- [ ] Create `SalesOrders` module.
- [ ] Create `PurchaseOrders` module.
- [ ] Create `Invoices` module.

## Phase 6: Activity & Service Modules
- [ ] Create `Activity` module (Meetings, Calls, Tasks schemas and logic).
- [ ] Create `Service` module (Tickets schema and logic).

## Phase 7: API Documentation & Validation
- [x] Configure Swagger in `main.ts`.
- [x] Set up global ValidationPipe in `main.ts`.
- [ ] Add Swagger decorators to all controllers and DTOs.
